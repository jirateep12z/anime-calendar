...
import { BookmarkButton } from '@/features/anime-bookmarks/client';
import {
  UseInAppNotificationFallback,
  UseNotifications
} from '@/features/anime-notifications/client';
import {
  BROADCAST_STATUSES,
...
import { DEFAULT_SCHEDULE_FILTER } from '../constants/calendar';
import {
...
const AnimeDetailDialog = dynamic(
  () =>
    import('@/features/anime-schedule/client').then(
      module => module.AnimeDetailDialog
    ),
  { ssr: false }
);
...
export function CalendarClient({
  ...
  useEffect(() => {
    ...
    const initial_preferences = Object.freeze({
      ...hydrated_preferences,
      selected_date
    });

    StartTransition(() => {
      set_preferences(initial_preferences);
    });
    PersistSchedulePreferences(window.localStorage, initial_preferences);
    void SyncAdultPreference(
      initial_preferences.is_adult_confirmed,
      initial_preferences.filter.is_adult_content_visible
    );
  ...
  const UpdatePreferences = useCallback(
    (next_preferences: SchedulePreferences) => {
      ...
      if (typeof window !== 'undefined') {
        PersistSchedulePreferences(window.localStorage, immutable_preferences);
      }
    },
    []
  );
  ...
  const visible_detail_model = visible_release_entry
    ? CreateAnimeDetailFromSchedule(visible_release_entry, now_seconds)
    : null;

import { FetchAniListMediaCatalog } from '@/features/anime-schedule/client';
...
export async function RefreshBookmarkCatalog(
  ...
  refreshed_entries.forEach(catalog_entry => {
    if (anilist_media_ids.has(catalog_entry.anilist_media_id)) {
      ...
      has_merged_entry = true;
    }
  });

  return has_merged_entry ? merged_entries : cached_entries;
}

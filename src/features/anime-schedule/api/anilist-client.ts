...
const ANIME_MEDIA_STATUSES = new Set([
  'RELEASING',
  'FINISHED',
  'NOT_YET_RELEASED',
  'CANCELLED',
...
function ParseAnimeMediaStatus(media_status: string | null): AnimeMediaStatus {
  return media_status !== null && ANIME_MEDIA_STATUSES.has(media_status)
    ? (media_status as Exclude<AnimeMediaStatus, null>)
    : null;
}
...
async function RequestBookmarkCatalog(
  anilist_media_ids: readonly number[],
  caller_signal: AbortSignal
): Promise<readonly AnimeCatalogEntry[]> {
  ThrowIfAborted(caller_signal);
  ...
  return Object.freeze(
    parsed_response.data.data.page.media.map(media =>
      Object.freeze({
        ...
        media_status: ParseAnimeMediaStatus(media.status),

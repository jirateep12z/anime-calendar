...
export function FormatScheduleCountdown(
  ...
  if (broadcast_status === 'UPCOMING') {
    return `เริ่มในอีก ${FormatCountdownDuration(schedule_entry.airing_at - now_seconds)}`;
  }

  if (broadcast_status === 'AIRING') {
    return `จบในอีก ${FormatCountdownDuration(schedule_entry.ends_at - now_seconds)}`;
  }

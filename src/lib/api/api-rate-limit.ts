...
import { ReadRateLimitSecret } from './rate-limit-environment';
...
export async function EnforceDeviceRateLimit(
  input: RateLimitKeyInput,
...
export function EnforceRequestRateLimit(
  ...
  return EnforceDeviceRateLimit({
    rate_limit_class,
    device_id,
    client_ip: ReadClientIp(request),
    rate_limit_secret: ReadRateLimitSecret()
  });
}
